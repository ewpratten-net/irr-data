...
def parse_routerconf_line(raw_line: str, own_as: int) -> RouterconfStatement:
    ...
    return RouterconfStatement(
        peer_as=int(matches.group(1).replace("AS", "")) if not matches.group(5) else matches.group(5),
        ...
        password = os.environ[f"PEER_PASS_{matches.group(1).upper()}"] if matches.group(4) else None,
        rewrite_as_to = int(matches.group(1).replace("AS", "")) if matches.group(5) else None
    )